...
void mr_train_batch3(int itask, KeyValue *kv, void *ptr)
{
    ...
    //vector<vector<float> > vvFeature(NVECSPERRANK, vector<float> (NDIMEN, 0.0));
    DMatrix data;
    data = initMatrix();
    data = createMatrix(NVECSPERRANK, NDIMEN);
    if (!validMatrix(data)) {
    ...
    float f = 0.0f;    
    for (uint64_t k = 0; k < NVECSPERRANK; k++) {
        for (unsigned int j = 0; j < NDIMEN; j++) {
            for (unsigned int i = 0; i < SZFLOAT; i++) {
                c1[i] = *((fdata + itask*NDIMEN*SZFLOAT*NVECSPERRANK) + k*NDIMEN*SZFLOAT + j*SZFLOAT + i);
            }
            f = *(float *)c1;
            data.rows[k][j] = f;
        }
...
void mr_train_batch(int itask, char *file, KeyValue *kv, void *ptr)
{
    ...
    if (SHUFFLE) {
        vector<vector<float> > vvFeature(NVECSPERRANK, vector<float> (NDIMEN));
        //vector<uint64_t> vRowIdx;
        vector<uint64_t> vRowIdx(NVECSPERRANK);
        for (uint64_t row = 0; row < NVECSPERRANK; row++) { 
