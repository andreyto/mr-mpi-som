...
/* -------------------------------------------------------------------------- */
{
    ...
    */
    ...
    if (bSPARSE) {
        ...
        assert(MMAPBINFILE.is_open());
        FDATA = reinterpret_cast<float*>((char*)MMAPBINFILE.data());   
        NVECSPERRANK = ceil(NVECS / NBLOCKS);
        NVECSLEFT = NVECS % NBLOCKS;
    }
    ...
    for (uint32_t n = rowStart; n < rowEnd+1; n++) {
        ...
        for (size_t y = 0; y < SOM_Y; y++) {
            for (size_t x = 0; x < SOM_X; x++) {
                ...
                uint32_t pos = (INDEXSPARSE + n)->position;
                   ...
                   unsigned int distance_metric)
{
    float distance = 0.0f;
    if (bSPARSE) {
        uint32_t pos = (INDEXSPARSE+rownum)->position;
...
float* get_wvec(size_t y,
                size_t x)
{
    FLOAT_T* wvec = (FLOAT_T*)malloc(SZFLOAT * NDIMEN);
    for (size_t d = 0; d < NDIMEN; d++)
        wvec[d] = CODEBOOK[y][x][d]; /// CAUTION: (y,x) order
    return wvec;
}
...
int read_matrix(const char *filename, const char *idxfilename)
{
    ///
